...
window.appendModule({
    run: async () => {
        if (window.innerWidth > 1024) {
            await waitForRender(() => document.querySelector(".app__aside__desktop + .app__main .header__tools"));
            addDesktopSettings();
        } else {
            await waitForRender(() => document.querySelector(".more-popup.list-modal"));
            addMobileSettings();
        }
    },
    onlyOnReloads: true,
