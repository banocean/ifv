...
export class SelectorRenderer {
    ...
    #createSelector(dayName) {
        ...
        element.querySelector("img:last-of-type")
            .addEventListener("click", () => this.#setSiblingDay(1))
    ...
    #getDaysDropdowns() {
        return Array.from(document.querySelectorAll(".app__content .MuiPaper-root")).map(
            (element) => ({
                element,
    ...
    #isWeekChanged() {
        return !this.firstDayName || document.querySelector(".app__content .MuiPaper-root .MuiAccordionSummary-content > h2")?.innerText !== this.firstDayName
    }
