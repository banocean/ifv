...
const fetchPatches = async () => {
  ...
  return JSON.parse(await patchesResponse.text());
}

const getConfig = async () => (await chrome.storage.sync.get("options"))?.options ?? {};

chrome.storage.onChanged.addListener((changes, namespace) => {
  if (!changes.options?.oldValue || changes.options?.oldValue === changes.options?.newValue) return
...
const getPatchesFiles = (patches, config) => {
  return patches.flatMap((patch) => {
    ...
    if (config[patch.name].enable) {
      if (patch.files?.js?.length) result.push(patch.files.js.map((file) => `patches/${file}`));
      if ((!patch.allowedHostsCss || patch.allowedHostsCss.includes(window.location.hostname)) && patch.files?.css?.length) {
        result.push(patch.files.css.map((file) => `patches/${file}`))
      }
    }
    return result
  }).flat()
...
async function run() {
  ...
  for (const filePath of getPatchesFiles(patches, config)) {
    const element = document.createElement(filePath.endsWith(".js") ? "script" : "link")
    element.setAttribute(filePath.endsWith(".js") ? "src" : "href", chrome.runtime.getURL(filePath))
    if (filePath.endsWith(".css")) element.setAttribute("rel", "stylesheet")
